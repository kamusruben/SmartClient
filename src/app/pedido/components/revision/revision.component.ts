...
export class RevisionComponent implements OnInit, AfterViewInit {
  ...
  public tempPedidosFiltrados: Pedido[] = [];
  ...
  constructor(private fb: FormBuilder, private _catalogoSevice: CatalogosService, private dateAdapter: DateAdapter<any>) {
  ...
  ngOnInit(): void {
    ...
    this.pedidosTodos.forEach((item)=>{
      ...
      // }
      /* Contenedores */
      // if(!this.contenedores.includes(item.numeroContenedor)){
      //   this.contenedores.push(item.numeroContenedor);
      // }
    });
  }
  ...
  public filtro(valor: any, tipo: string):void{
    ...
    if(value != estadoProgramacionEnum.TODOS && value != '') {
      if((prop == 'numeroContenedor' || prop == 'prefactAduana') && value == 'N/A') {
  ...
  public aplicarFiltros(){
    ...
    if(rCodigo != null && rCodigo != estadoProgramacionEnum.TODOS) {
      ...
      if(representante != null) {
        this.pedidosFiltrados = this.pedidosFiltrados.filter(x => x.refRep == representante.nombre);
      }
    }
    //filtro Cliente
    ...
    if(clCodigo != null && clCodigo != estadoProgramacionEnum.TODOS) {
      ...
      if(cliente != null) {
        this.pedidosFiltrados = this.pedidosFiltrados.filter(x => x.cliente == cliente.nombre);
      }
    }
    //filtro Producto
    ...
    if(prCodigo != null && prCodigo != '') {
      ...
      if(producto != null) {
        this.pedidosFiltrados = this.pedidosFiltrados.filter(x => x.producto == producto.nombre);
      }
    }
    //filtro Naviera
    ...
    if(navCodigo != null && navCodigo != estadoProgramacionEnum.TODOS) {
      ...
      if(naviera != null) {
        this.pedidosFiltrados = this.pedidosFiltrados.filter(x => x.naviera == naviera.nombre);
      }
    }
    //filtro Naviera
    ...
    if(marCodigo != null && marCodigo != estadoProgramacionEnum.TODOS) {
      ...
      if(marca != null) {
        this.pedidosFiltrados = this.pedidosFiltrados.filter(x => x.marca == marca.nombre);
      }
    }
    //filtro Contenedor
